					...
					<div class="order-details-container">
						...
						<div class="order-details-container-details">
							...
							<div class="notice-payment">
								<p><strong>Important:</strong> This is a one-time payment for <strong>a private service, not affiliated with any government agency.</strong> Government filing fees are not included.</p>
							</div>
						</div>
					</div>
...
<?php
	...
	public function handle_get_payment_details(WP_REST_Request $request)
	{
		...
		$payload  = $this->get_checkout_payment_session_payload($entry);
		if (!$payload) {
			return new WP_REST_Response(array('message' => 'Failed to create payment session payload.'), 500);
		}
		$response = array(
	...
	function get_checkout_payment_session_payload($entry)
	{
		// --- Extract needed data from $entry ---
		$amount = $this->get_amount_export(rgar($entry, 'payment_amount'), $entry['currency']) * 100; // Checkout.com takes amount in cents (e.g., $1.00 = 100)
		$currency = isset($entry['currency']) ? $entry['currency'] : 'USD';
		$reference = isset($entry['133']) ? $entry['133'] : 'ORD-' . time();
		$first_name = isset($entry['1.3']) ? $entry['1.3'] : '';
		$middle_name = isset($entry['1.4']) ? $entry['1.4'] : '';
		$last_name = isset($entry['1.6']) ? $entry['1.6'] : '';
		$full_name = trim("$first_name $middle_name $last_name");

		$email = isset($entry['2']) ? $entry['2'] : '';
		$country = isset($entry['12.6']) ? $entry['12.6'] : 'India';

		// --- Prepare payload for Checkout.com ---
		$payload = [
			"amount" => $amount, // e.g. 100 = $1.00
			"currency" => $currency,
			"reference" => $reference,
			"display_name" => "My Passport Express",
			"billing" => [
				"address" => [
					"country" => $this->get_country_code($country) // Convert to ISO if possible, e.g., "India" -> "IN"
				]
			],
			"customer" => [
				"name" => $full_name,
	...
	public function process_sitea_callback(WP_REST_Request $request)
	{
		...
		error_log("event type is here: " . $event_type);
		if (empty($event_type)) {
		...
		error_log("payment data is here: " . print_r($payment_data, true));
		if (! $payment_data) {
		...
		error_log("entry id is here: " . $entry_id);
		if (! $entry_id) {
		...
		$entry = GFAPI::get_entry($entry_id);
		error_log("entry is here: " . print_r($entry, true));
		if (is_wp_error($entry) || ! $entry) {
		...
		if (in_array(rgar($entry, 'payment_status'), array('Paid'), true)) {
		...
		error_log("action is here: " . print_r($action, true));
		$this->process_payment_action($action);
